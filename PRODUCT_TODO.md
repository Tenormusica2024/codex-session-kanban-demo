...
- Follow-up added `/` search focus, `?` workflow-guide toggle, `Esc` guide close, `x` clear-filters, candidate-list filtering, filter result summary, expanded attention filters, candidate-review quick filters, selected-card URL hashes, copy-card links, and browser smoke coverage for these review shortcuts.
