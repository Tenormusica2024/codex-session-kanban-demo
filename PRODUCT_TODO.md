...
- Follow-up made the GitHub Pages push workflow run desktop, mobile, and narrow browser smoke checks before deployment, while keeping manual `run_browser_smoke=true` available for workflow-dispatch testing.
...
Risk to avoid:
- Keep the browser smoke bounded to public fixture artifacts and avoid logged-in or subjective visual checks in CI. Use CiC for authenticated or qualitative review.
...
- Follow-up added a 320px narrow-viewport smoke mode and ultra-narrow CSS polish for spacing, actions, and dropzones.
...
- Follow-up added a latest-phase-over-stale-context fixture based on private operational validation: a session can start with mail-link investigation but later move to X bookmark recommendation dedupe, and extraction should keep the latest task rather than reverting to the entry topic.
...
- Follow-up added `/` search focus, `?` workflow-guide toggle, `Esc` guide close, `x` clear-filters, candidate-list filtering, filter result summary, expanded attention filters, candidate-review quick filters, selected-card URL hashes, copy-card links, copy-card briefs, the `b` card-brief shortcut, and browser smoke coverage for these review shortcuts.
